...
//
// Add a GraphicObject to Renderer
void Renderer::add_drawable(std::string name, RendererLayer layer, std::shared_ptr<GraphicObject> drawable)
{
    std::cout << "Adding GraphicObject: " << name << std::endl;
    if(drawable) m_drawables.insert(std::make_pair(std::make_pair(name, layer), drawable));
}
//
// Delete a GraphicObject from Renderer
void Renderer::delete_drawable(std::string name, RendererLayer layer)
{
    std::map<std::pair<std::string, RendererLayer>, std::shared_ptr<GraphicObject> >::iterator it = m_drawables.find(std::make_pair(name, layer));

    if(it == m_drawables.end()) // Not found
    {
        std::cout << "Cannot delete, GraphicObject " << name << " doesn't exist" << std::endl;
    }
    else
    {
...
{
    for(std::map<std::pair<std::string, RendererLayer>, std::shared_ptr<GraphicObject> >::const_iterator it = m_drawables.begin();
        it != m_drawables.end();
        ++it)
        {
